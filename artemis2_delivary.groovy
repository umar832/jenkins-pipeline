de {
	properties(
		...
		stage("Authenticate"){
			timestamps {
				ws {
					sh '''
					ssh centos@dev1.awsumar.com  $(aws ecr get-login --no-include-email --region us-east-1)
						'''
				}
			}
		}
			...
			timestamps {
				ws {
					try {
						sh '''
							#!/bin/bash
							IMAGES=$(ssh centos@dev1.awsumar.com docker ps -aq) 
							for i in \$IMAGES; do
								ssh centos@dev1.awsumar.com docker stop \$i
								ssh centos@dev1.awsumar docker rm \$i
							done 
							'''
					} catch(e) {
	...
	stage("Run Container"){
		timestamps {
			ws {
				sh '''
					ssh centos@dev1.awsumar.com docker run -dti -p 5001:5000 965334959964.dkr.ecr.us-east-1.amazonaws.com/artemis:${Version}
					'''
            }
        }
    }
